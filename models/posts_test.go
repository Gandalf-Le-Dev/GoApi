...
func testPostsExists(t *testing.T) {
	...
	e, err := PostExists(ctx, tx, o.PostID)
	if err != nil {
...
func testPostsFind(t *testing.T) {
	...
	postFound, err := FindPost(ctx, tx, o.PostID)
	if err != nil {
		t.Error(err)
	}
...
func testPostToManyAddOpComments(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.PostID != first.PostID {
			t.Error("foreign key was wrong value", a.PostID, first.PostID)
		}
		if a.PostID != second.PostID {
			t.Error("foreign key was wrong value", a.PostID, second.PostID)
		}
...
func testPostToOneCategoryUsingCategory(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCategory(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPostToOneSetOpCategoryUsingCategory(t *testing.T) {
	var err error
...
func testPostToOneRemoveOpCategoryUsingCategory(t *testing.T) {
	var err error
...
var (
	postDBTypes = map[string]string{`PostID`: `integer`, `UserID`: `integer`, `CategoryID`: `integer`, `Title`: `character varying`, `Content`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
