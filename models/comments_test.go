...
func testCommentsExists(t *testing.T) {
	...
	e, err := CommentExists(ctx, tx, o.CommentID)
	if err != nil {
...
func testCommentsFind(t *testing.T) {
	...
	commentFound, err := FindComment(ctx, tx, o.CommentID)
	if err != nil {
		t.Error(err)
	}
...
func testCommentToOnePostUsingPost(t *testing.T) {
	...
	if check.PostID != foreign.PostID {
		t.Errorf("want: %v, got %v", foreign.PostID, check.PostID)
	}
...
func testCommentToOneSetOpPostUsingPost(t *testing.T) {
	...
	for i, x := range []*Post{&b, &c} {
		...
		if a.PostID != x.PostID {
			t.Error("foreign key was wrong value", a.PostID, x.PostID)
		}
	}
}
...
var (
	commentDBTypes = map[string]string{`CommentID`: `integer`, `PostID`: `integer`, `UserID`: `integer`, `Content`: `text`, `CreatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
